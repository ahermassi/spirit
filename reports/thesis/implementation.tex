...
\label{sec:poses}
  \section{Terminology}
    ...
    The orientation of the drone is represented by a quaternion, \sym{quat}, and defined in the \gls{jpl} convention as:
    ...
    A quaternion can be converted into Euler angles using: \cite{blanco2013}
	...
	\arctan{\frac{2\left(\sym{quatw}\sym{quatx} + \sym{quaty}\sym{quatz}\right)}
		     {1 - 2\left(\sym{quatx}^2 + \sym{quaty}^2\right)}} \\
	\arcsin{\left(2\left(\sym{quatw}\sym{quaty} - \sym{quatz}\sym{quatx}\right)\right)} \\
	\arctan{\frac{2\left(\sym{quatw}\sym{quatz} + \sym{quatx}\sym{quaty}\right)}
		     {1 - 2\left(\sym{quaty}^2 + \sym{quatz}^2\right)}} \\
      \end{bmatrix}
    \end{equation}
  ...
  \section{Relative poses}
    ...
    and $\sym{quat} = \begin{bmatrix}\sym{quatw} & \sym{quatim}\end{bmatrix}^\top$. The solution is:
...
\chapter{Visualization}
  ...
  \section{Rendering}
	...
	\begin{bmatrix}\sym{quatx} & \sym{quaty} & \sym{quatz}\end{bmatrix},& \text{if } \sym{rotang} \bmod 180^\circ = 0 \\
    ...
    \begin{equation}
      \begin{bmatrix}\sym{quatw} & \sym{quatx} & \sym{quaty} & \sym{quatz}\end{bmatrix} \mapsto
      \begin{bmatrix}\sym{quatw} & -\sym{quatx} & \sym{quatz} & \sym{quaty}\end{bmatrix}
    \end{equation}
